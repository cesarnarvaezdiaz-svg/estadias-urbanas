...
- Flujo OAuth con Google mediante credencial nativa validada en servidor.
- Inicio de sesión con Facebook validado en servidor.
...
- Checkout Pro de Mercado Pago abierto en el navegador seguro del sistema, no en WebView, con URL de retorno a la pantalla de verificación de la app.
...
Google OAuth usa autorización nativa con validación del `id_token` en el servidor. Facebook valida el `access_token` contra Graph API y Apple intercambia el código de autorización en el servidor. Todos requieren credenciales reales en los portales de Google, Meta y Apple; el código no inventa ni incorpora secretos. En el servidor define también `FACEBOOK_APP_ID` y, recomendado, `FACEBOOK_APP_SECRET`.
